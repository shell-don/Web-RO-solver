...
import { EXEMPLES } from "./exemples/exemples.js";
...
function fillEnonceExemple(key) {
    getEl("enonce_exemple").innerHTML = gen_html_exemple(key);
}

function reset() {
    getEl("enonce_exemple").innerHTML = "";
    getEl("mode_output").innerHTML = "";
    getEl("parametres_et_contraintes_output").innerHTML = "";
    getEl("resultat_output").innerHTML = "";
    getEl("coefficients_fonction_objective").innerHTML = "";
    getEl("mode_lineaire").checked = false;
    getEl("mode_quadratique").checked = false;
}

function loadExemple(key) {
    // si la clé est vide, reset
    if (!key) {
        reset();
        return;
    }
    // sinon récupérer l'exemple
    const ex = EXEMPLES[key];
    // si l'exemple est introuvable dans EXEMPLES, ne rien faire
    if (!ex) return;
